...
class __TwigTemplate_e48f6eb46baaa2d1c2d05c72b30dd864660af6a7efd45a5ac8363b592e0a8f22 extends Twig_Template
{
    private $source;
...
<script>
    ...
    \$('#recharge_order_sup').on('click',function(e){
        ...
        echo "',11,'setSuccessDome','type_success','置成功确认',460,270,true);
        ...
        // line 76
        echo $this->extensions['Symfony\Bridge\Twig\Extension\RoutingExtension']->getPath("admin_orders_edit_status_sup");
        echo "',14,'setFailureDome','type_fail','置失败确认',0,0,true);
                },
                settlement: function(){
    ...
    table.on('tool(order-table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter=\"对应的值\"
        ...
        // line 108
        echo $this->extensions['Symfony\Bridge\Twig\Extension\RoutingExtension']->getPath("admin_orders_edit_status_sup");
        echo "',6,'setSuccessDome','type_success','置成功确认',460,270,false,data.id);
        }
...
";
    ...
    {
        return array (  157 => 112,  150 => 108,  129 => 90,  112 => 76,  104 => 71,  32 => 18,  29 => 1,);
    }
...
<script>
    ...
    \$('#recharge_order_sup').on('click',function(e){
        ...
        if (target.is('.reverse-select') || target.parents().is('.reverse-select')) {
            ...
            var active = {
                setSuccess: function(){
                    //按钮【确定】的回调
                    confirmAction('{{ path(\"admin_orders_edit_status_sup\") }}',11,'setSuccessDome','type_success','置成功确认',460,270,true);

                },
                setFailure: function(){
                    //按钮【确定】的回调
                    confirmAction('{{ path(\"admin_orders_edit_status_sup\") }}',14,'setFailureDome','type_fail','置失败确认',0,0,true);
                },
                settlement: function(){
                    var yes = function(index){
                        //按钮【确定】的回调
    ...
    table.on('tool(order-table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter=\"对应的值\"
        ...
        var data = obj.data;
        if(layEvent === 'mark_succeed'){ //置成功
            confirmAction('{{ path(\"admin_orders_edit_status_sup\") }}',6,'setSuccessDome','type_success','置成功确认',460,270,false,data.id);
        }

        if(layEvent === 'mark_failed'){ //置失败
            confirmAction('{{ path(\"admin_orders_edit_status_sup\") }}',7,'setFailureDome','type_fail','置失败确认',0,0,false,data.id);
        }
