...
class __TwigTemplate_d152f7dce2e41c97fc985ec0866dea236027dcad534241649fe829f1acd0ace5 extends Twig_Template
{
    private $source;
        ...
        if(domstatus=='hidden'){
            layer.prompt({title: '请输入密码验证身份', formType:1,type:1}, function(pass, index){
                ...
                \$.post('{{ path('admin_user_details_open_platform_get_key',{id:user.user_id}) }}',{
                    '_csrf_token':'{{ csrf_token('admin_user_details_open_platform_get_key') }}',
                    password:password
                },function(data){
                    if(data.status){
                        \$('.app-secret').html(data.msg+'　');
