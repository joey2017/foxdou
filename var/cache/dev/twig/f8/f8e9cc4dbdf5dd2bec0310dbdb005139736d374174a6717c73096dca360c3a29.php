...
class __TwigTemplate_ba6629d8c1fc46944c90ccc20f80f15edd008b235b7ffb2d4a0607f72cec200b extends Twig_Template
{
    private $source;
  ...
  <script type=\"text/javascript\">
      function sub(dom) {
          ...
          \$.ajax('{{ path('reseller_recharge_orders_checkout') }}',{
              type:'post',
              data:{_csrf_token:'{{ csrf_token('reseller_recharge_orders_checkout') }}', order_id:'{{ order.id|default(0) }}', password:password},
              success:function (data) {
                  if(!data.status){
                      \$('#msg').text(data.msg);
                      \$(dom).removeAttr('disabled');
