...
class OrderController extends AdminBaseController {
     ...
     */
    public function rechargeAction(Request $request, OrderService $orderManager, Form $form) {
        if ($request->query->count() <= 2) {
            ...
            return $this->render("Admin/Order/recharge.twig",
                [
                    ...
                    'order_mark'                => $request->query->getInt('order_mark'),
        ...
        $form->init([
            ...
            'settle_status'     => ["digit"],
            'settlement_status' => ["digit"],
            //'start_time'        => ["date"],
            //'end_time'          => ["date"],
            'range_time'        => [""],
            'summary'           => ["enum[0|1]"],
            'page'              => ['natural'],
            'limit'             => ["enum[10|25|50|100]"]
        ]);
        ...
        list($rows, $total) = $orderManager->getRechargeOrders($form, $form->get('page', 1), $form->get('limit', 10));

        return ["status" => true, "total" => $total, 'data' => $rows, 'msg' => '获取成功'];
        //return ["code" => 0, "count" => $total, "data" => $rows, 'msg' => '获取成功'];
    }
    ...
    public function getOrderMarkTotalAction(Request $request, $id = 0) {
        ...
        $sql    = "SELECT order_mark,count(id) AS total FROM yzb_order_recharge WHERE product_cate_id=? GROUP BY order_mark";
        $stmt   = OrderRecharge::query($sql, [$product_cate_id]);
    ...
    public function getOrderRechargeTypeTotalAction($ajax = true) {
        $data              = [];
        $data['recharge']  = $this->_getOrderTypeTotal('product_cate_id', 'yzb_order_recharge');
        $data['suppliers'] = $this->_getOrderTypeTotal('source_product_cate_id', 'yzb_order_recharge_sup');
    ...
    public function emptyChannelAddOrderAction(Request $request) {
        ...
        try {
            ...
            $this->logger->error("修改空渠道可下单渠道状态失败", $text);
        }
     ...
     */
    public function suppliersAction(Request $request, OrderService $orderManager, Form $form) {
        ...
        $cates            = join("|", array_keys(Constants::PRODUCT_CATEGORIES));
        $settleStatuses   = join("|", array_keys(OrderRechargeSup::SETTLE_STATUS));
    ...
    /** 改变主订单状态
     * @Route("/orders/edit-status", name="admin_orders_edit_status")
    ...
    public function editStatusAction(Request $request) {
        ...
        if (!($ids && in_array($order_status, [OrderRecharge::STATUS_COMPLETED_MAN, OrderRecharge::STATUS_FAILED_MAN]))) {
        ...
        if ($order_status == OrderRecharge::STATUS_FAILED_MAN) {
         ...
         * 2.主订单置失败（主订单状态=充值中，充值成功（系统），充值成功（人工））
         */
        $count = 0;
        ...
        foreach ($orders as $order) {
            ...
            if ($order_status == OrderRecharge::STATUS_COMPLETED_MAN) {//置成功
                ...
                case 1:// 最新SUP订单置成功并结算
                        ...
                        OrderRechargeSup::transaction(function () use ($sup_order, $request) {
                            OrderRechargeSup::update_all(
                                [
                                    ...
                                    'settle_status'   => OrderRechargeSup::SETTLE_STATUS_ADMIN_SUCCESS_DONE,
                            ...
                            OrderRechargeSupOperationLog::update_all(
                                [
                                    'recharge_status'   => OrderRechargeSup::STATUS_SUCCEED_ADMIN,
                                    'order_status'      => OrderRecharge::STATUS_COMPLETED_MAN,
                                    'settle_status'     => OrderRechargeSup::SETTLE_STATUS_ADMIN_SUCCESS_DONE,
                    ...
                    $record = UserMoneyLog::exists(
                        [
                            ...
                            'order_id' => $order['id'],
                            'log_type' => UserMoneyLog::getLogTypeCode($order['platform_id'], UserMoneyLog::TYPE_ORDER_SETTLEMENT)
                        ]
                    );
                    if (empty($record)) {
                        ...
                        OrderRechargeSup::transaction(function () use ($sup_order, $request) {
                            OrderRechargeSup::update_all(
                                [
                                    'recharge_status' => OrderRechargeSup::STATUS_SUCCEED_ADMIN,
                                    'edit_time'       => date('Y-m-d H:i:s'),
                                    'settle_status'   => OrderRechargeSup::SETTLE_STATUS_ADMIN_SUCCESS_PENDING
                                ],
                                ['id' => $sup_order->id]
                            );
            ...
            } elseif ($order_status == OrderRecharge::STATUS_FAILED_MAN) {//置失败（需要验证操作密码）

                if (!in_array($order['status'], [OrderRecharge::STATUS_PROCESSING, OrderRecharge::STATUS_COMPLETED_SYS, OrderRecharge::STATUS_COMPLETED_MAN])) {
                    continue;
                }
                ...
                $user_money_record = UserMoneyLog::exists(
                    [
                        'order_id' => $order['id'],
                        'user_id'  => $order['user_id'],
                        'log_type' => UserMoneyLog::getLogTypeCode($order['platform_id'], UserMoneyLog::TYPE_REFUND)
                    ]
                );
                ...
                case 1:// 最新SUP订单置失败并结算
                    ...
                    if (!(in_array($sup_order->recharge_status, [OrderRechargeSup::STATUS_FAILED_SYS, OrderRechargeSup::STATUS_FAILED_SUP, OrderRechargeSup::STATUS_FAILED_ADMIN]) && $sup_order->settle_status == OrderRechargeSup::SETTLE_STATUS_PENDING)) {
                        try {
                            OrderRechargeSup::transaction(function () use ($sup_order, $request) {
                                ...
                                OrderRechargeSup::update_all(
                                    [
                                        'settle_status' => OrderRechargeSup::SETTLE_STATUS_SUP_FAIL_UNPAID,
                                        'edit_time'     => date('Y-m-d H:i:s')
                                    ],
                                    ['id' => $sup_order->id]
                                );
                    ...
                    if (!(in_array($sup_order->recharge_status, [OrderRechargeSup::STATUS_FAILED_SYS, OrderRechargeSup::STATUS_FAILED_SUP, OrderRechargeSup::STATUS_FAILED_ADMIN]) && in_array($sup_order->settle_status, [OrderRechargeSup::SETTLE_STATUS_SYSTEM_SUCCESS_DONE, OrderRechargeSup::SETTLE_STATUS_SUP_SUCCESS_DONE, OrderRechargeSup::SETTLE_STATUS_ADMIN_SUCCESS_DONE]))) {
                        try {
                            OrderRechargeSup::transaction(function () use ($sup_order, $request) {
                                //更新SUP订单结算状态为总后台人工失败，已退款结算
                                OrderRechargeSup::update_all(
                                    [
                                        'settle_status' => OrderRechargeSup::SETTLE_STATUS_ADMIN_FAIL_REFUND,
                                        'edit_time'     => date('Y-m-d H:i:s')
                                    ],
                                    ['id' => $sup_order->id]
                                );
                        ...
                        $user_money_record = UserMoneyLog::exists(
                            [
                                'order_id' => $order['id'],
                                'user_id'  => $sup_order->sup_user_id,
                                'log_type' => UserMoneyLog::getLogTypeCode($order['platform_id'], UserMoneyLog::TYPE_DEDUCTIONS)
                            ]
                        );
    ...
    public function setSuccessAction(Request $request) {
        ...
        if (!($ids && $order_status == OrderRecharge::STATUS_COMPLETED_MAN)) {
            return ['status' => false, 'msg' => '参数有误'];
        }
    ...
    protected function addUserMoneyLog($log_type, $order_id, $sup_user_id, $platform_id, $amount, $user_id, $product_name, $note, OrderService $orderService = null) {

        $user_money_record = UserMoneyLog::exists(
            [
                'order_id' => $order_id,
                'user_id'  => $sup_user_id,
                'log_type' => UserMoneyLog::getLogTypeCode($platform_id, $log_type)
            ]
        );
    ...
    /** 改变SUP订单状态
     * @Route("/orders/edit-status-sup", name="admin_orders_edit_status_sup")
     * $status=[0=>未知,1=>充值中,2=>充值成功(系统),3=>充值失败(系统),4=>待付款,5=>已取消,6=>充值成功(人工),7=>充值失败(人工)];
     * $rechage_status=[0 => 未知 , 1 => 待付款 , 2 => 待提单 , 3 => 已提单 , 4 => 已验单 , 5 => 提交可疑 , 6 => 提交失败 , 7 => 充值中 , 8 => 充值可疑 , 9 => 充值成功 (系统) , 10 => 充值成功 (供应商人工) , 11 => 充值成功 (总后台人工) , 12 => 充值失败 (系统) , 13 => 充值失败 (供应商人工) , 14 => 充值失败 (总后台人工) , 15 => 已退款 , 16 => 已取消 , 17 => 待处理];
     * $settle_status=['0'=>'未知','1'=>'未结算','2'=>'已结算','3'=>'已退款'];
     *
     */
    public function editStatusSupAction(Request $request, OrderService $orderService) {
        $ids             = $request->request->get('id');
        $recharge_status = $request->request->getInt('order_status');
        $type            = $request->request->getInt('type');
        if (!($ids && in_array($recharge_status, [OrderRechargeSup::STATUS_SUCCEED_ADMIN, OrderRechargeSup::STATUS_FAILED_ADMIN]))) {
            return ['status' => false, 'msg' => '参数有误'];
        }
        ...
        if ($recharge_status == OrderRechargeSup::STATUS_FAILED_ADMIN) {
            $password = $request->request->get('password');
        ...
        foreach ($orders as $order) {
            ...
            if ($recharge_status == OrderRechargeSup::STATUS_SUCCEED_ADMIN) {//置成功
                    ...
                    if ($is_newest) {
                        try {
                            OrderRechargeSup::transaction(function () use ($request, $order_status, $order, $order_recharge, $recharge_status) {
                                ...
                                $data['settle_status']     = OrderRechargeSup::SETTLE_STATUS_ADMIN_SUCCESS_DONE;
                        ...
                        $user_money_record = UserMoneyLog::exists(
                            [
                                ...
                                'log_type' => UserMoneyLog::getLogTypeCode($order_recharge->platform_id, UserMoneyLog::TYPE_ORDER_SETTLEMENT)
                            ]
                        );
                        ...
                        $log_text = "结算SUP订单[{$order['id']}]，并对主订单[{$order['order_id']}]置成功";
                    }
                    ...
                    if ($is_newest) {
                        try {
                            OrderRechargeSup::transaction(function () use ($request, $order_status, $order, $order_recharge, $recharge_status) {
                                ...
                                $data['order_status']      = $order_status;
                                $data['recharge_status']   = $recharge_status;
                                $data['settle_status']     = OrderRechargeSup::SETTLE_STATUS_ADMIN_SUCCESS_PENDING;
                                $data['operation_user_id'] = $this->getAdminUser()->id;
                                $data['if_admin']          = OrderRechargeSupOperationLog::IS_ADMIN;
                                $data['operator_ip']       = $request->getClientIp();
                                $data['operation_type']    = OrderRechargeSupOperationLog::OPERATION_MANUAL;
                        ...
                        $log_text = "暂不结算SUP订单[{$order['id']}]，并对主订单[{$order['order_id']}]置成功";
                    }
                ...
                case 3:// 结算该笔SUP订单，并对主订单不进行任何操作
                    ...
                    $log_text = "结算SUP订单[{$order['id']}]，并对主订单[{$order['order_id']}]不进行任何操作";
                ...
                case 4:// 暂不结算该笔SUP订单，对主订单不进行任何操作
                    ...
                    $log_text = "暂不结算SUP订单[{$order['id']}]，对主订单[{$order['order_id']}]不进行任何操作";
            ...
            if ($recharge_status == OrderRechargeSup::STATUS_FAILED_ADMIN) {//置失败
                ...
                if ($order['settle_status'] == OrderRechargeSup::SETTLE_STATUS_PENDING) {//待结算
                    try {
                        OrderRechargeSup::transaction(function () use ($request, $order, $order_recharge, $recharge_status, $order_status) {
                            ...
                            OrderRechargeSup::update_all(
                                [
                                    'settle_status' => OrderRechargeSup::SETTLE_STATUS_ADMIN_FAIL_UNPAID,
                                    'end_time'      => date('Y-m-d H:i:s'),
                                    'edit_time'     => date('Y-m-d H:i:s')
                                ],
                                ['id' => $order['id']]
                            ...
                            $data['order_status']      = $order_status;
                            $data['recharge_status']   = $recharge_status;
                            $data['settle_status']     = OrderRechargeSup::SETTLE_STATUS_ADMIN_FAIL_UNPAID;
                            $data['operation_user_id'] = $this->getAdminUser()->id;
                            $data['if_admin']          = OrderRechargeSupOperationLog::IS_ADMIN;
                            $data['operator_ip']       = $request->getClientIp();
                            $data['operation_type']    = OrderRechargeSupOperationLog::OPERATION_MANUAL;
                    ...
                    if (in_array($order['settle_status'], [OrderRechargeSup::SETTLE_STATUS_SYSTEM_SUCCESS_DONE, OrderRechargeSup::SETTLE_STATUS_SUP_SUCCESS_DONE, OrderRechargeSup::SETTLE_STATUS_ADMIN_SUCCESS_DONE])) {//系统充值成功，已付款结算/供应商人工成功，已付款结算/总后台人工成功，已付款结算
                        try {
                            OrderRechargeSup::transaction(function () use ($request, $order, $order_recharge, $recharge_status, $order_status) {
                                // 更新SUP订单结算状态为总后台人工失败，不付款结算
                                OrderRechargeSup::update_all(
                        ...
                        // 是否存在对该供应商的最新一笔扣款资金流水
                        $user_money_record = UserMoneyLog::exists(['order_id' => $order['order_id'], 'user_id' => $order['sup_user_id'], 'log_type' => UserMoneyLog::getLogTypeCode($order_recharge->platform_id, UserMoneyLog::TYPE_DEDUCTIONS)]);
                ...
                case 1:// 对主订单置失败
                    if ($is_newest) {
                        ...
                        $log_text = "对SUP订单[{$order['id']}]置失败，对主订单[{$order['order_id']}]置失败";
                    }
                    break;
                case 2:// 对主订单不进行任何操作
                    $log_text = "对SUP订单[{$order['id']}]置失败，对主订单[{$order['order_id']}]不进行任何操作";
            ...
            try {
                $result = OrderRechargeSup::transaction(function () use ($recharge_status, $order, $operation, $request, $log_text) {
                    OrderRechargeSup::update_all(
                        [
                    ...
                    AdminOperationLog::create([
                        ...
                        'type'         => $operation,
                        'subject_type' => 'ORDER',
                        'info'         => "用户[{$this->getAdminUser()->user_name}]" . $log_text,
                        'ip'           => $request->getClientIp(),
                        'add_time'     => date('Y-m-d H:i:s'),
                    ]);
                    return true;
     ...
     * @Route("/orders/set-success-sup", name="admin_orders_set_success_sup")
     */
    public function setFailureSupAction(Request $request, OrderService $orderService) {
        $ids             = $request->request->get('id');
        $recharge_status = $request->request->getInt('order_status');
        $type            = $request->request->getInt('type');
        $password        = $request->request->get('password');
        ...
        foreach ($orders as $order) {
            ...
            if ($order['settle_status'] == OrderRechargeSup::SETTLE_STATUS_PENDING) {//待结算
                ...
                if (in_array($order['settle_status'], [OrderRechargeSup::SETTLE_STATUS_SYSTEM_SUCCESS_DONE, OrderRechargeSup::SETTLE_STATUS_SUP_SUCCESS_DONE, OrderRechargeSup::SETTLE_STATUS_ADMIN_SUCCESS_DONE])) {
                    try {
                        OrderRechargeSup::transaction(function () use ($request, $order, $order_recharge, $recharge_status, $order_status) {
                            // 更新SUP订单结算状态为总后台人工失败，不付款结算
                            OrderRechargeSup::update_all(
    ...
    public function settlementSupAction(Request $request) {
        ...
        $options = [
            ...
            'from'   => '`yzb_order_recharge` AS o',
            'joins'  => 'LEFT JOIN `yzb_order_recharge_sup` AS s ON s.order_id=o.id AND s.id=(SELECT max(id) FROM `yzb_order_recharge_sup` WHERE order_id=o.id)',
            'order'  => 'o.id DESC',
        ];
        ...
        foreach ($orders as $order) {
            if (!in_array($order['status'], [OrderRecharge::STATUS_COMPLETED_SYS, OrderRecharge::STATUS_COMPLETED_MAN])) {
                continue;
            }
     ...
     *
     */
    public function rechargeAgainAction(Request $request, OrderService $orderService) {
        ...
        $options = [
            ...
            'from'   => '`yzb_order_recharge` AS o',
            'joins'  => 'LEFT JOIN `yzb_order_recharge_sup` AS s ON s.order_id=o.id AND s.id=(SELECT max(id) FROM `yzb_order_recharge_sup` WHERE order_id=o.id) LEFT JOIN `yzb_user_money_trade_log` AS L ON l.order_id=o.id',
            'order'  => 'o.id DESC',
        ];

        $order = OrderRecharge::first(['id' => $id], $options);
        ...
        if ($order->sid && !in_array($order->status, [OrderRecharge::STATUS_PROCESSING, OrderRecharge::STATUS_COMPLETED_SYS, OrderRecharge::STATUS_COMPLETED_MAN])) {
        ...
        if ($again == 1) {//对当前最新SUP订单置失败并结算
            ...
            if (($order->status == OrderRecharge::STATUS_COMPLETED_SYS && $order->settle_status == OrderRechargeSup::SETTLE_STATUS_SYSTEM_SUCCESS_DONE) || ($order->status == OrderRecharge::STATUS_COMPLETED_MAN && in_array($order->settle_status, [OrderRechargeSup::SETTLE_STATUS_SUP_SUCCESS_DONE, OrderRechargeSup::SETTLE_STATUS_ADMIN_SUCCESS_DONE]))) {
                $user_money_trade = UserMoneyTradeLog::last(['order_id' => $order->id, 'other_party_user_id' => $order['sup_user_id']]);
                $platform_user    = $this->getPlatformAccountInfo($order->platform_id, $order['sup_user_id']);
                try {
            ...
            if ($order->settlement_status < OrderRecharge::SETTLEMENT_COMPLETED) {
                ...
                try {
                    UserMoneyLog::transaction(function () use ($order, $platform_user) {
                        ...
                        OrderRecharge::update_all(
                            [
                                'status'            => OrderRecharge::STATUS_COMPLETED_MAN,
    ...
    public function getSupOrderStatusAction(Request $request) {
        ...
        $options   = [
            ...
            'from'   => '`yzb_order_recharge` AS o',
            'joins'  => 'LEFT JOIN `yzb_order_recharge_sup` AS s ON s.order_id = o.id',
            'order'  => 'o.id DESC',
        ];
    ...
    public function detailAction(Request $request) {
        ...
        $select   = 'o.product_cate_id,o.order_cate_id,o.id,o.product_name,o.isp_id,o.province_name,o.city_name,o.product_face_value,o.recharge_method,o.note,
