...
class RecaptchaService extends BaseService {
     ...
     * @return array
     */
    public function newRecaptcha($id = '_default',$config = []) {
        $generator = new RecaptchaGenerator($config);
        list($info, $image) = $generator->create($id);
        $this->session->set("recaptcha:{$id}", $info);

        return [$info, $image];
    }

    public function verifyRecaptcha($code, $id = '_default', $clear = false) {
        $key           = "recaptcha:{$id}";
        $recaptchaInfo = $this->session->get($key);
        if($clear) {
            $this->session->remove($key);
        }
        if(!$recaptchaInfo || $recaptchaInfo['id'] !== $id || $recaptchaInfo['code'] !== md5(strtoupper($code)) || $recaptchaInfo['expires_at'] < time()) {
            return false;
        }
    ...
    public function clear($id = '_default') {
        $this->session->remove('recaptcha');
    }
}
...
class RecaptchaGenerator
{
    ...
    public function create($id = '')
    {
        ...
        // 验证码使用随机字体
        $ttfPath = __DIR__ . '/../Resources/recaptcha/' . ($this->useZh ? 'zhttfs' : 'ttfs') . '/';
        ...
        // 保存验证码
        $secode['id']         = $id;
        $secode['code']       = md5(strtoupper(join('', $code)));
        $secode['expires_at'] = time() + $this->config['expire'];
