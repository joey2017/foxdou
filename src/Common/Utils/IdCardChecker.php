...
class IdCardChecker extends Identity {
    public static $idcard_status = [0 => '未知', 1 => '正常', 2 => '待补齐', 3 => '黑名单'];
    /*原接口验证错误代码*/
    private $old_api_error_code = [
        '0200' => '证件格式不合法',
        '0201' => '证件格式不合法',
        '0202' => '证件格式不合法',
        '0203' => '很抱歉，您的证件号码为黑名单，暂不能办理',
        '9999' => '很抱歉，您的证件号码为黑名单，暂不能办理',
        '0204' => '很抱歉，您的证件号码办理的号码数过多，暂不能办理',
        '0205' => '很抱歉，您的证件号码下有号码为欠费，暂不能办理',
    ...
    const area_all = [
        ...
        '659001' => '新疆维吾尔族自治区直辖县级行政单位石河子市',
        '659002' => '新疆维吾尔族自治区直辖县级行政单位阿拉尔市',
        '659003' => '新疆维吾尔族自治区直辖县级行政单位图木舒克市',
        '659004' => '新疆维吾尔族自治区直辖县级行政单位五家渠市',
    ];
    ...
    public static function isValid($card_id) {
        ...
        if (strlen($card_id) == 18) {//18位身份号码检测
            $last_check_code = [1, 0, 'X', 9, 8, 7, 6, 5, 4, 3, 2,];
            //地区检验
            $check_area = self::getArea($card_id);
            if (empty($check_area['province'] || empty($check['area']))) {
                return false;
            }
    ...
    public static function getArea($card_id) {
        $area_key           = (int)substr($card_id, 0, 2);
        $area_all_key       = (int)substr($card_id, 0, 6);
        $result['province'] = empty(self::area[$area_key]) ? null : self::area[$area_key];
        $result['area']     = empty(self::area_all[$area_all_key]) ? null : self::area_all[$area_all_key];
        return $result;
    }
     ...
     * 获取身份证号基本信息
     */
    public static function getInfoByCard($card_id, $verifyFormat = false) {
        if ($verifyFormat) {
            if (!self::isValid($card_id)) {
                return null;
            }
        }

        $result        = self::getArea($card_id);
        $tyear         = substr($card_id, 6, 4);
        $tmonth        = substr($card_id, 10, 2);
        $tday          = substr($card_id, 12, 2);
        $sexint        = (int)substr($card_id, 16, 1);
        ...
        if ($tyear > date("Y") || $tyear < (date("Y") - 100)) {
            ...
            $result['tdate'] = $tyear . "-" . $tmonth . "-" . $tday;//生日日期
