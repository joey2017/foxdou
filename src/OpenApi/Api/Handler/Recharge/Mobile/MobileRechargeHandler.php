...
use App\OpenApi\Api\Handler\Recharge\ApiError;
use App\OpenApi\ApiContext;
use App\OpenApi\EnqueueCommands;
use App\OpenApi\Form\Recharge\Mobile\RechargeForm;
 ...
 */
class MobileRechargeHandler extends AbstractHandler {
    ...
    public function init(ApiContext $context) {
        ...
        if($this->isDebug() && $context->request->query->has('debug')) {
    ...
    public function handle(ApiContext $context) {
        ...
        // 创建订单，失败会抛出异常
        // 因为目前快速上线版没有供应商，所以 source 参数传递 null 进去
        $order = $this->orderService()->createRechargeOrder($context->account, $product, null, $numberInfo, $amount, 0, $context->form->outer_sn, OrderRecharge::STATUS_PROCESSING, '');
    ...
    public function finish(ApiContext $context) {
        $providerId = $context->order->provider_id;
        if($this->isDebug() && $context->parameters->getBoolean('debug') && ($debugProviderId  = $context->parameters->get('provider_id'))) {
            $this->logger()->debug('当前处于调试模式并且存在 provider_id 参数，使用参数指定的 provider_id：' . $debugProviderId);
            $providerId = $debugProviderId;
        }

        $jobInfo  = ['sup_order_id' => $context->order->current_sup_order_id, 'provider_id' => $providerId];
        $producer = $this->producer();
        $producer->sendCommand(EnqueueCommands::RECHARGE_SUBMIT_ORDER, $jobInfo);
    }
    ...
    private function findMostMatchingProduct(ApiContext $context, Form $form, $numberInfo) {
        ...
        $products    = $this->rechargeProductService()->findProducts($context->account, $numberInfo, $productType, $amount, $speed, $productId);
